...
func (query *NetworkVersionInfoQuery) _CostQueryMakeRequest(client *Client) (_ProtoRequest, error) {
	...
	pb.NetworkGetVersionInfo.Header.Payment = &proto.Transaction{
...
func (query *NetworkVersionInfoQuery) IsFrozen() bool {
	return query._IsFrozen()
}

// Sign uses the provided privateKey to sign the transaction.
func (query *NetworkVersionInfoQuery) Sign(
	privateKey PrivateKey,
) *NetworkVersionInfoQuery {
	return query.SignWith(privateKey.PublicKey(), privateKey.Sign)
}

func (query *NetworkVersionInfoQuery) SignWithOperator(
	client *Client,
) (*NetworkVersionInfoQuery, error) {
...
func (query *NetworkVersionInfoQuery) SignWith(
	publicKey PublicKey,
...
func (query *NetworkVersionInfoQuery) Freeze() (*NetworkVersionInfoQuery, error) {
	return query.FreezeWith(nil)
}

func (query *NetworkVersionInfoQuery) FreezeWith(client *Client) (*NetworkVersionInfoQuery, error) {
	if query.IsFrozen() {
		return query, nil
	}
	if query.actualCost.AsTinybar() == 0 {
		if query.queryPayment.tinybar != 0 {
			...
			actualCost, err := query.GetCost(client)
			if err != nil {
				return &NetworkVersionInfoQuery{}, err
			}

			if query.actualCost.tinybar < actualCost.tinybar {
				return &NetworkVersionInfoQuery{}, ErrMaxQueryPaymentExceeded{
					QueryCost:       actualCost,
