...
// DelegatableContractIDFromString constructs a ContractID from a string formatted as `Shard.Realm.Contract` (for example "0.0.3")
func DelegatableContractIDFromString(data string) (DelegatableContractID, error) {
	shard, realm, num, checksum, _, evm, err := _IdFromString(data)
	if err != nil {
...
// DelegatableContractIDFromSolidityAddress constructs a ContractID from a string representation of a _Solidity address
...
func (id DelegatableContractID) String() string {
	if id.EvmAddress != nil {
		return fmt.Sprintf("%d.%d.%s", id.Shard, id.Realm, *id.EvmAddress)
	}
...
func (id DelegatableContractID) _ToProtobuf() *services.ContractID {
	if id.EvmAddress != nil {
		temp, _ := hex.DecodeString(*id.EvmAddress)
		return &services.ContractID{
			ShardNum: int64(id.Shard),
			RealmNum: int64(id.Realm),
			Contract: &services.ContractID_EvmAddress{EvmAddress: temp},
		}
	}
	...
	return &services.ContractID{
		ShardNum: int64(id.Shard),
		RealmNum: int64(id.Realm),
		Contract: &services.ContractID_ContractNum{ContractNum: int64(id.Contract)},
	}
}
...
func _DelegatableContractIDFromProtobuf(contractID *services.ContractID) *DelegatableContractID {
	...
	case *services.ContractID_ContractNum:
		return &DelegatableContractID{
			...
			Contract: uint64(id.ContractNum),
		}
	...
	default:
		panic("(BUG) non-exhaustive switch statement")
	}
}
