...
func (transaction *TokenRevokeKycTransaction) Execute(
	...
	if !transaction.IsFrozen() {
		_, err := transaction.FreezeWith(client)
		if err != nil {
			return TransactionResponse{}, errors.Wrap(err, "FreezeWith in SignWithOperator")
		}
	}
...
func (transaction *TokenRevokeKycTransaction) FreezeWith(client *Client) (*TokenRevokeKycTransaction, error) {
	...
	if err := transaction.initTransactionID(client); err != nil {
		return transaction, errors.Wrap(err, "initTransactionID in TokenRevokeKycTransaction.FreezeWith")
	}
