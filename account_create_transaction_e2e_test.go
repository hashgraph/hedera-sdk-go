...
func TestIntegrationAccountCreateWithEvmAddress(t *testing.T) {
	env := NewIntegrationTestEnv(t)
	...
	evmAddressAccount, err := AccountIDFromEvmPublicAddress(publicKey.ToEvmAddress())

	resp, err := NewAccountCreateTransaction().
		SetEvmAddress(*evmAddressAccount.EvmAddress).
		SetNodeAccountIDs(env.NodeAccountIDs).
		Execute(env.Client)
...
func TestIntegrationAccountCreateWithAliasKeyECDSA(t *testing.T) {
	env := NewIntegrationTestEnv(t)
	privateKey, err := PrivateKeyGenerateEcdsa()
	if err != nil {
		println(err.Error())
...
func TestIntegrationAccountCreateWithAliasKeyEd25519(t *testing.T) {
	env := NewIntegrationTestEnv(t)
