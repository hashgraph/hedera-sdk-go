...
func TestIntegrationTokenCreateTransactionWithRoyaltyCannotExceedOne(t *testing.T) {
	...
	assert.Error(t, err)
	if err != nil {
		assert.Equal(t, fmt.Sprint("exceptional receipt status: ROYALTY_FRACTION_CANNOT_EXCEED_ONE"), err.Error())
	}
...
func TestIntegrationTokenCreateTransactionRoyaltyFeeOnlyAllowedForNonFungibleUnique(t *testing.T) {
	...
	assert.Error(t, err)
	if err != nil {
		assert.Equal(t, fmt.Sprint("exceptional receipt status: CUSTOM_ROYALTY_FEE_ONLY_ALLOWED_FOR_NON_FUNGIBLE_UNIQUE"), err.Error())
	}
